...
local TopPanel = function(s, offset)
	...
	local panel = wibox
	{
		...
		y = s.geometry.y,
		stretch = false,
		-- bg = beautiful.xresources.get_current_theme().background,
...
--     })
    ...
    if hide_bar then
        local bar_activator = wibox({x=panel.x,height=1,width=panel.width,visible=true,ontop=true,bg=beautiful.xresources.get_current_theme().background,below=true})

        bar_activator:connect_signal("mouse::enter",function()
            panel.visible = true;panel.height=dpi(28);panel:struts{top=dpi(28)}
        end)

        panel:connect_signal("mouse::leave",function()
            panel.visible = false ;panel.height=dpi(1);panel:struts{top=dpi(0)}
        end)
	...
	s.clock_tooltip = awful.tooltip
	{
		...
		timer_function = function()
			...
			-- llocal date_str = 'Today is the ' ..
			-- '<b>' .. day .. ordinal ..
			-- ' of ' .. month .. '</b>.\n' ..
			-- 'And it\'s ' .. os.date('%A')

            local date_str = '<b>' .. day .. ordinal ..
			' ' .. month .. '</b>.\n' ..
			 os.date('%A')
